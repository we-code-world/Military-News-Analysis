...
!function (e) {
    ...
    i = {
        ...
        constructor: t, _init: function (t) {
            ...
            }, e.each(R.defaults, function (i, t) {
            ...
            a.showPreview && (a.$modal = e(n), a.$modal && a.$modal.length || (t = e(document.createElement("div")).html(r).insertAfter(a.$container), a.$modal = e(n).insertBefore(t), t.remove()), i.initModal(a.$modal), a.$modal.html(a._getModalContent()), e.each(i.MODAL_EVENTS, function (e, i) {
            ...
            n = y.attr("data-template") || "generic", r = y.find(".kv-file-content"), o = r.length ? r.html() : "", u = t.data("caption") || "", f = t.data("size") || "", l = u + " " + f, v.find(".kv-zoom-title").html(l), s = v.find(".kv-zoom-body"), v.removeClass("kv-single-content"), a ? (p = s.addClass("file-thumb-loading").clone().insertAfter(s), s.html(o).hide(), p.fadeOut("fast", function () {
        ...
        }, _generatePreviewTemplate: function (t, a, r, n, o, l, s, d, c, p, u) {
            var f, m = this, g = m.slug(r), v = "", h = m.previewSettings[t] || m.defaults.previewSettings[t],
                w = h && h.width ? h.width : "", _ = h && h.height ? h.height : "",
                b = c || m._renderFileFooter(g, s, i.isEmpty(w) ? "auto" : w, l), C = m._getPreviewIcon(r),
                y = "type-default", E = C && m.preferIconicPreview, x = C && m.preferIconicZoomPreview,
                T = function (a, l, s, c) {
                    var f = s ? "zoom-" + o : o, v = m._getPreviewTemplate(a), h = (d || "") + " " + c;
                    return m.frameClass && (h = m.frameClass + " " + h), s && (h = h.replace(" " + i.SORT_CSS, "")), v = m._parseFilePreviewIcon(v, r), "text" === a && (l = i.htmlEncode(l)), "object" !== t || n || e.each(m.defaults.fileTypeSettings, function (e, i) {
                        "object" !== e && "other" !== e && i(r, n) && (y = "type-" + e)
                    }), v.replace(/\{previewId}/g, f).replace(/\{caption}/g, g).replace(/\{frameClass}/g, h).replace(/\{type}/g, n).replace(/\{fileindex}/g, p).replace(/\{width}/g, w).replace(/\{height}/g, _).replace(/\{typeCss}/g, y).replace(/\{footer}/g, b).replace(/\{data}/g, l).replace(/\{template}/g, u || t)
                };
            return p = p || o.slice(o.lastIndexOf("-") + 1), m.fileActionSettings.showZoom && (v = T(x ? "other" : t, a, !0, "kv-zoom-thumb")), v = "\n" + m._getLayoutTemplate("zoomCache").replace("{zoomContent}", v), f = T(E ? "other" : t, a, !1, "kv-preview-thumb"), f + v
            ...
            r.loadedImages = [], r.totalImagesCount = 0, e.each(t, function (e, i) {
                ...
                var E, x, T, S, F, I, P, k, z, A, $ = g + e, D = u + "-" + $, U = t[e],
                    j = U.name ? r.slug(U.name) : "", R = (U.size || 0) / 1e3, M = "", L = i.objUrl.createObjectURL(U),
                    Z = 0, O = "";
                if (h > 0) for (S = 0; h > S; S++) k = v[S], z = r.msgFileTypes[k] || k, O += 0 === S ? z : ", " + z;
                ...
                if (i.isEmpty(w) || (M = new RegExp("\\.(" + w.join("|") + ")$", "i")), T = R.toFixed(2), r.maxFileSize > 0 && R > r.maxFileSize) return F = r.msgSizeTooLarge.replace("{name}", j).replace("{size}", T).replace("{maxSize}", r.maxFileSize), void (r.isError = y(F, U, D, e));
                if (null !== r.minFileSize && R <= i.getNum(r.minFileSize)) return F = r.msgSizeTooSmall.replace("{name}", j).replace("{size}", T).replace("{minSize}", r.minFileSize), void (r.isError = y(F, U, D, e));
                if (!i.isEmpty(v) && i.isArray(v)) {
                    for (S = 0; S < v.length; S += 1) I = v[S], A = m[I], Z += A && "function" == typeof A && A(U.type, U.name) ? 1 : 0;
        ...
        }, _validateImage: function (e, i, t, a) {
            var r, n, o, l = this, s = l.$preview, d = s.find("#" + e), c = d.attr("data-fileindex"), p = d.find("img");
            i = i || "Untitled", p.length && l._handler(p, "load", function () {
                n = d.width(), o = s.width(), n > o && (p.css("width", "100%"), d.css("width", "97%")), r = {
                    ind: c,
                    id: e
                }, l._checkDimensions(c, "Small", p, d, i, "Width", r), l._checkDimensions(c, "Small", p, d, i, "Height", r), l.resizeImage || (l._checkDimensions(c, "Large", p, d, i, "Width", r), l._checkDimensions(c, "Large", p, d, i, "Height", r)), l._raise("fileimageloaded", [e]), l.loadedImages.push({
                    ind: c,
                    img: p,
                    thumb: d,
                    pid: e,
                    ...
                    siz: a,
                    validated: !1
                }), l._validateAllImages()
            })
        }, _validateAllImages: function () {
            var e, i, t, a = this, r = {val: 0}, n = a.loadedImages.length, o = a.resizeIfSizeMoreThan;
            if (n === a.totalImagesCount && (a._raise("fileimagesloaded"), a.resizeImage)) for (e = 0; e < a.loadedImages.length; e++) i = a.loadedImages[e], i.validated || (t = i.siz, t && t > 1e3 * o && a._getResizedImage(i, r, n), a.loadedImages[e].validated = !0)
            ...
            var n, o, l, s, d = [], c = arguments.length > 1, p = a.isUploadable,
                u = c ? t.originalEvent.dataTransfer.files : r.get(0).files, f = a.filestack.length,
                m = i.isEmpty(r.attr("multiple")), g = m && f > 0, v = 0, h = a._getFileIds(),
                w = function (i, t, r, n) {
                    var o = e.extend(!0, {}, a._getOutData({}, {}, u), {id: r, index: n}),
                        l = {id: r, index: n, file: t, files: u};
            ...
            })), i.isEmpty(d) || 0 === d.length) return p || a.clear(), a._showFolderError(v), void a._raise("fileselectnone");
            if (a._resetErrors(), s = d.length, o = a._getFileCount(a.isUploadable ? a.getFileStack().length + s : s), a.maxFileCount > 0 && o > a.maxFileCount) {
                if (!a.autoReplace || s > a.maxFileCount) return l = a.autoReplace && s > a.maxFileCount ? s : o, n = a.msgFilesTooMany.replace("{m}", a.maxFileCount).replace("{n}", l), a.isError = w(n, null, null, null), a.$captionContainer.find(".kv-caption-icon").hide(), a._setCaption("", !0), void a.$container.removeClass("file-input-new file-input-ajax-new");
            ...
            var t = this, a = 0, r = [], n = [], o = [];
            t._getThumbs().each(function () {
                var l, s = e(this), d = s.attr("data-fileindex"), c = t.filestack[d], p = s.attr("id");
                "-1" !== d && -1 !== d && (void 0 !== c ? (r[a] = c, n[a] = t._getFileName(c), o[a] = t._getFileId(c), s.attr({
                    id: t.previewInitId + "-" + a,
                    "data-fileindex": a
                }), a++) : (l = "uploaded-" + i.uniqId(), s.attr({
                    id: l,
                    "data-fileindex": "-1"
                }), t.$preview.find("#zoom-" + p).attr("id", "zoom-" + l)))
